...
function selfTest() {
  ...
  ok(codes(r).includes('site_never_names_shop'), '#16333 the school district page never names S R Tobacco', codes(r));
  ...
  ok(!r.instant && codes(r).includes('domain_too_new') && codes(r).includes('domain_after_import'),
    '#278 gulfportcigars.com was re-registered after we imported the listing', codes(r));
  ...
  r = vary({ store: { ...good.store, id: 16, name: 'Tobacco Shop', city: 'Hilo', website: 'towercigars.com' },
    page: { text: 'nothing in particular' } });
  ...
  ok(!r.instant && r.reasons.length >= 3 && r.reasons.every(x => x.label), 'a listing with no website collects reasons, not a refusal', codes(r));
