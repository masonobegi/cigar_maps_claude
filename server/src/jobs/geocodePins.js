...
/**
 * The words that say what KIND of street it is, folded to one spelling. A
 * record writes "518 Bradley St" and the shop's own website writes "518 Bradley
 * Street"; without this they are two different streets, siteBacksAddress never
 * fires, and the automatic tier — which requires it — is dead on arrival.
 *
 * The type is folded, not dropped: "1 Main St" and "1 Main Ave" really are two
 * different streets, and dropping the word would merge them.
 */
const STREET_TYPE_FOLD = {
  street: 'st', str: 'st', avenue: 'ave', av: 'ave', road: 'rd', boulevard: 'blvd',
  drive: 'dr', lane: 'ln', court: 'ct', place: 'pl', parkway: 'pkwy', pky: 'pkwy',
  highway: 'hwy', terrace: 'ter', circle: 'cir', square: 'sq', trail: 'trl',
  plaza: 'plz', turnpike: 'tpke', expressway: 'expy', freeway: 'fwy',
...
function streetKey(address) {
  return String(address || '')
    ...
    .split(' ').map(w => STREET_TYPE_FOLD[w] || w).join(' ');
}
 ...
 */
/**
 * Every time zone the United States keeps, including its territories. Anything
 * else is another country's clock — and several of them, Canada's above all,
 * sit under "America/".
 */
...
function foreignVerdict(row) {
  ...
  // "America/Toronto" starts with America/, so a prefix test cannot see that it
  // is Canadian. The US zones are a short list; anything else is somebody
  // else's clock, whichever continent it is named after.
  if (row.pin_zone && !US_TIME_ZONES.has(row.pin_zone)) signals.push(`its pin stands in ${row.pin_zone}`);
...
function selfTest() {
  ...
  // The suite goes, and the direction words fold to one spelling, so "570 W
  // 1730 S" and "570 West 1730 South" are the same street rather than two.
  ok('street key drops the suite', streetKey('570 West 1730 South Suite 900'), 'w 1730 s');
  ok('and folds the spelling out to the abbreviation',
    streetKey('570 W 1730 S'), streetKey('570 West 1730 South'));
  ok('but two different street types stay different',
    streetKey('1 Main St') === streetKey('1 Main Ave'), false);
...
/**
 * The writing half, against a real database. selfTest above judges the rules on
 * fixtures; this checks what actually reaches the table — provenance, the
 * claimed and staff-edited guard, the edit log, and that a hide is reversible.
 * Needs PGLITE_DIR pointing at a scratch database.
 */
async function applyTest() {
  ...
  const mk = async (o) => (await db.get(
    ...
    [o.name, o.address || '120 Elm St', o.city || 'Dayton', o.state || 'OH', o.zip || '45402',
      o.phone || null, o.lat ?? 39.0, o.lng ?? -84.0, o.claimed ?? 0, o.staff_edited ?? 0,
      o.timezone || 'America/New_York'])).id;
  ...
  const abroad = await mk({ name: 'Windsor Cigars', city: 'Windsor', state: 'ON', zip: 'N9A 1A1', phone: '(519) 254-0017' });

  const move = (id) => ({ id, reason: 'both geocoders put it 90 km from our pin',
    to: { lat: 39.7684, lng: -86.1581 } });
  const file = require('path').join(require('os').tmpdir(), `geopins_apply_${process.pid}.json`);
  fs.writeFileSync(file, JSON.stringify([
    move(plain), move(owned), move(staffed),
    { id: abroad, kind: 'foreign', reason: 'a Canadian postcode and a Canadian area code' },
  ]));
  ...
  ok(Math.abs(Number(a.lat) - 39.7684) < 1e-6, 'an ordinary listing has its pin moved', a.lat);
  ...
  ok(log.some(e => e.field === 'lat' && Number(e.before) === 39 && e.job === 'geocodePins'),
    'and the old coordinates are in the edit log, so the move can be undone', log.map(e => e.field));

  ok(Number((await after(owned)).lat) === 39, 'a claimed listing is not touched by the sweep');
  ok(Number((await after(staffed)).lat) === 39, 'nor is a staff-edited one');

  const f = await after(abroad);
  ok(f.visible === 0 && /Canadian/.test(f.storefront_reason || ''),
    'a foreign row is hidden, with the reason recorded', { visible: f.visible, why: f.storefront_reason });
  ok(f.name === 'Windsor Cigars', 'and is hidden, never deleted');

  for (const id of [plain, owned, staffed, abroad]) await db.run('DELETE FROM stores WHERE id = ?', [id]);
  console.log(`\ngeocodePins apply-test: ${pass} passed, ${fail} failed`);
  return fail;
}

module.exports = {
  geocode, nominatim, decide, apply, selfTest, applyTest,
