...
router.post('/register', (req, res) => {
  ...
  ).run(email, password_hash, name, account_type || 'user');
...
router.get('/me', requireAuth, (req, res) => {
  const user = db.prepare('SELECT id, email, name, account_type, avatar_url, bio, created_at FROM users WHERE id = ?').get(req.user.id);
  if (!user) return res.status(404).json({ error: 'User not found' });

  let store = null;
  if (user.account_type === 'store') {
    store = db.prepare('SELECT * FROM stores WHERE user_id = ?').get(user.id);
  }
  res.json({ user, store });
});
...
router.put('/me', requireAuth, (req, res) => {
  ...
  db.prepare('UPDATE users SET name = ?, bio = ?, avatar_url = ? WHERE id = ?')
    .run(name, bio, avatar_url, req.user.id);
