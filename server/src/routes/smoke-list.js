...
// Update smoke list item
router.put('/:id', requireAuth, (req, res) => {
  const item = db.prepare('SELECT id FROM smoke_list WHERE id = ? AND user_id = ?').get(req.params.id, req.user.id);
...
// Mark as smoked (quick action)
router.post('/:id/mark-smoked', requireAuth, (req, res) => {
  const item = db.prepare('SELECT id FROM smoke_list WHERE id = ? AND user_id = ?').get(req.params.id, req.user.id);
  if (!item) return res.status(404).json({ error: 'Not found' });

  db.prepare("UPDATE smoke_list SET status='smoked', smoked_on=DATE('now') WHERE id=?").run(req.params.id);
  res.json({ success: true });
});

// Remove from smoke list
router.delete('/:id', requireAuth, (req, res) => {
  db.prepare('DELETE FROM smoke_list WHERE id = ? AND user_id = ?').run(req.params.id, req.user.id);
  res.json({ success: true });
});
